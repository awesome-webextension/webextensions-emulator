...
export function makeListener (listenersArea) {
  return {
    addListener: listener => {
      ...
      if (!listenersArea.some(fn => fn === listener)) {
        listenersArea.push(listener)
      }
    },
    removeListener: listener => {
      ...
      const index = listenersArea.findIndex(fn => fn === listener)
      if (index >= 0) {
        listenersArea.splic(0, 1)
      }
    },
    hasListener: listener => {
      ...
      return listenersArea.some(fn => fn === listener)
    }
  }
}
